...
for i in range(1):
    ...
    threshold = 5
    amount = 1
    blurred = cv2.GaussianBlur(fin, (window_size, window_size), sigma, sigma)
    lowContrastMask = abs(fin - blurred) < threshold
    sharpened = fin*(1+amount) + blurred*(-amount)
    np.copyto(sharpened, fin, where = lowContrastMask)
        
    path = "../output/" + "pyr_" + file_name + "_sharpened"
